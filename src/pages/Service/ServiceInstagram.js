...
class ServiceInstagram extends Component {
    ...
    handleSubmit = (event) => {
        ...
        let InstagramUser = this.state.user.toLowerCase();
        ...
        if (this.state.user === '') {
            alert('Debe ingresar el usuario de Instagram a obtener datos');
            return;
        }    
        if (fi > ff) {
        ...
        if (this.state.num_paginas > 300 || this.state.num_paginas < 1){
            alert('Debe ingresar numero de paginas entre 1 y 300');
            return;
        }
        ...
        Axios.post("/api5/instagramScraping",{
            fechai: fechaini,
            fechaf: fechafin,
